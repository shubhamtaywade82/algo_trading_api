...
module InstrumentHelpers
  ...
  def resolve_instrument_code
    # Try to get the enum value directly (e.g., 'INDEX' from index enum key)
    code = if respond_to?(:instrument_code_before_type_cast) && instrument_code_before_type_cast.present?
             instrument_code_before_type_cast.to_s.upcase
           elsif respond_to?(:instrument_code) && instrument_code.present?
             # instrument_code enum returns the value (e.g., 'INDEX')
             instrument_code.to_s.upcase
           end

    # Fallback to instrument_type
    code ||= instrument_type.presence if respond_to?(:instrument_type)
