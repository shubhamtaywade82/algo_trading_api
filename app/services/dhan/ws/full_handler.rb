  ...
  module Ws
    class FullHandler
      ...
      def self.call(bytes)
        ...
        high_oi  = bytes[38, 4].pack('C*').unpack1('L<')
        low_oi   = bytes[42, 4].pack('C*').unpack1('L<')
        open_v   = bytes[46, 4].pack('C*').unpack1('e')
        close_v  = bytes[50, 4].pack('C*').unpack1('e')
        high_v   = bytes[54, 4].pack('C*').unpack1('e')
        low_v    = bytes[58, 4].pack('C*').unpack1('e')
        inst = Instrument.find_by(security_id: sid) or return
        tick_time = Time.at(bytes[14, 4].pack('C*').unpack1('L<'))
