...
module Trading
  ...
  #
  # Option chain structure (from Option::ChainAnalyzer):
  #   chain[:oc] is a Hash keyed by strike price strings
  #   chain[:oc]["21000.000000"] => { "ce" => { "oi" => N, ... }, "pe" => { "oi" => N, ... } }
  class DirectionResolver < ApplicationService
    ...
    # Chain structure: chain[:oc] is Hash[strike_string => { "ce" => { "oi" => N }, "pe" => { "oi" => N } }]
    def calculate_oi_bias
      ...
      if pe_oi > ce_oi
        :bullish  # put writers dominate = bullish
      elsif ce_oi > pe_oi
        :bearish  # call writers dominate = bearish
      else
        :neutral
      end
    end
