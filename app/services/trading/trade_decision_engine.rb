  ...
  class TradeDecisionEngine < ApplicationService
    ...
    def call
      ...
      if regime.state == :no_trade
        return no_trade_with(regime.reason, symbol: @symbol, expiry: expiry_to_use,
                                            iv_rank: iv_rank_pct, spot: spot, regime: regime)
      end

      dir_result = Trading::DirectionResolver.call(spot: spot, candles: candles, option_chain: chain)
      unless dir_result.direction
        return no_trade_with("No directional signal: #{dir_result.reason}", symbol: @symbol,
                                                                            expiry: expiry_to_use, iv_rank: iv_rank_pct,
      ...
      unless entry_check.valid
        return no_trade_with("Entry not confirmed: #{entry_check.reason}", symbol: @symbol,
                                                                           direction: direction, expiry: expiry_to_use,
                                                                           iv_rank: iv_rank_pct, spot: spot, regime: regime)
      end
      ...
      unless analysis[:proceed]
        return no_trade_with("Chain analysis blocked: #{analysis[:reason]}", symbol: @symbol,
                                                                             direction: direction, expiry: expiry_to_use,
                                                                             iv_rank: iv_rank_pct, spot: spot, regime: regime,
