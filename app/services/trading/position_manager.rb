...
module Trading
  ...
  class PositionManager < ApplicationService
    ...
    def call
      ...
      case @action
      when :move_sl_to_be then move_sl_to_break_even(position)
      when :trail_sl      then trail_stop_loss(position)
      when :partial_exit  then partial_exit(position)
      when :force_exit    then force_exit(position)
