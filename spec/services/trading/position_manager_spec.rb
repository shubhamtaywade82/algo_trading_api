...
RSpec.describe Trading::PositionManager, type: :service do
  ...
  before do
    allow(Positions::ActiveCache).to receive(:fetch)
      .with(security_id, exchange_segment)
      .and_return(position)
  end
  ...
  describe ':move_sl_to_be' do
    it 'calls Orders::Adjuster with entry price and returns success' do
      adjuster_double = instance_double(Orders::Adjuster, call: true)
      allow(Orders::Adjuster).to receive(:new).with(position, { trigger_price: 120.0 }).and_return(adjuster_double)
      ...
      expect(Orders::Adjuster).to have_received(:new).with(position, { trigger_price: 120.0 })
  ...
  describe ':trail_sl' do
    ...
    it 'computes new trigger at default 5% below LTP for long position' do
      ...
      allow(Orders::Adjuster).to receive(:new).with(position, { trigger_price: expected_trigger }).and_return(adjuster_double)
      ...
      expect(Orders::Adjuster).to have_received(:new).with(position, { trigger_price: expected_trigger })
      expect(result.success).to be true
