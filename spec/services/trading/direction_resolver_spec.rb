  ...
  describe '#call' do
    context 'when both price and OI biases are bullish' do
      it 'returns CE direction' do
        # spot > vwap (candles close below spot) + PE OI > CE OI (put writers dominate = bullish)
        candles = build_candles(close: 21900.0)  # spot (22100) > VWAP (~21900) = bullish price
        chain   = build_chain(ce_oi: 100_000, pe_oi: 500_000)  # PE > CE OI = bullish
    ...
    context 'when both price and OI biases are bearish' do
      it 'returns PE direction' do
        # spot < vwap (candles close above spot) + CE OI > PE OI (call writers dominate = bearish)
        candles = build_candles(close: 22300.0)  # spot (22100) < VWAP (~22300) = bearish price
        chain   = build_chain(ce_oi: 500_000, pe_oi: 100_000)  # CE > PE OI = bearish
        result  = described_class.call(spot: spot, candles: candles, option_chain: chain)
    ...
    context 'when all candles have zero volume' do
      it 'falls back to simple average for VWAP' do
        # Zero-volume candles; VWAP should use simple average of closes
        candles = build_candles(close: 21900.0, volume: 0)
        chain   = build_chain(ce_oi: 100_000, pe_oi: 500_000)
        result  = described_class.call(spot: spot, candles: candles, option_chain: chain)
        # Simple avg close is ~21900; spot 22100 > 21900 so price_bias = bullish
        expect(result.price_bias).to eq(:bullish)
      end
    end
