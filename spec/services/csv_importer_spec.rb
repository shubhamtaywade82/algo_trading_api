...
RSpec.describe CsvImporter, type: :class do
  ...
  before do
    ...
    allow(described_class).to receive(:download_csv).and_return(sample_csv_path)
  end

  describe '.import' do
    it 'imports all data successfully' do
      expect { described_class.import }.to change(Instrument, :count).by(5)
                                                                     .and change(Derivative, :count).by(2)
                                                                                                    .and change(
                                                                                                      MarginRequirement, :count
                                                                                                    ).by(5)
        .and change(
          OrderFeature, :count
        ).by(5)
